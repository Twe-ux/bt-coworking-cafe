...
export async function middleware(req: NextRequest) {
  ...
  const userRole = token?.role?.slug as 'dev' | 'admin' | 'staff' | 'client' | undefined;
  ...
  if (authRoutes.includes(pathname)) {
    if (isAuthenticated) {
      ...
      } else if (userRole === 'dev' || userRole === 'admin' || userRole === 'staff') {
  ...
  if (adminDashboardPattern.test(pathname)) {
    ...
    if (userRole === 'client') {
      console.log('❌ Client trying to access admin dashboard, redirecting to client dashboard');
      if (username) {
        return NextResponse.redirect(new URL(`/${username}`, req.url));
      }
  ...
  if (clientDashboardPattern.test(pathname) && !publicRoutes.includes(pathname)) {
    ...
    if (userRole === 'client') {
      if (pathUsername !== username) {
        console.log(`❌ Client trying to access another user's dashboard (${pathUsername} != ${username})`);
        return NextResponse.redirect(new URL(`/${username}`, req.url));
      }
    }
